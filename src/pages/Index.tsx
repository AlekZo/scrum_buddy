...
import { getProjectSetting, PROJECT_COLORS } from "@/lib/project-settings";
import { PresentationModeProvider, usePresentationMode } from "@/lib/presentation-mode";
...
const IndexInner = () => {
  ...
  // Find the latest entry before selectedDate that has "doing" content (for prefill over weekends)
  const latestPreviousEntry = useMemo(() => {
    const sorted = entries
      .filter((e) => e.date < selectedDate && e.doing?.trim())
  ...
  // Get active project color for header tint
  const activeProjectIndex = data.projects.indexOf(activeProject);
  const activeProjectSettings = activeProject ? getProjectSetting(activeProject) : {};
  const activeProjectColor = activeProjectSettings.color || (activeProjectIndex >= 0 ? PROJECT_COLORS[activeProjectIndex % PROJECT_COLORS.length] : undefined);
  ...
  const mainContent = (
    <>
      ...
      {/* Header */}
      <header
        className="border-b border-border/50 backdrop-blur-sm sticky top-0 z-10 transition-colors duration-300"
        ref={headerRef}
        style={{
          backgroundColor: activeProjectColor ? `${activeProjectColor.replace(')', ' / 0.08)')}` : undefined,
          borderBottomColor: activeProjectColor ? `${activeProjectColor.replace(')', ' / 0.2)')}` : undefined,
        }}
      >
              ...
              <TooltipProvider>
                <Tooltip>
                  ...
                  <TooltipContent side="bottom" className="text-xs">
                    {presentationMode ? "Exit presentation mode — show hours" : "Hide all hours for screen sharing"}
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
              <ThemeToggle />
          ...
          <Tabs defaultValue="log" className="space-y-0">
            ...
            <div className="sticky z-[9] bg-background/95 backdrop-blur-sm py-3 border-b border-border/30" style={{ top: headerHeight }}>
              <TabsList className="w-full sm:w-auto overflow-x-auto overflow-y-hidden justify-start sm:justify-center no-scrollbar">
                ...
                <TabsTrigger value="tasks" className="flex-1 sm:flex-none min-h-[44px] sm:min-h-[36px] text-xs sm:text-sm">Tasks</TabsTrigger>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 space-y-4">
                  <EntryForm
                    ...
                    yesterday={yesterday}
                    latestPreviousEntry={latestPreviousEntry}
                    planData={planData}
                    onSave={saveEntry}
            ...
            <TabsContent value="tasks" className="mt-4">
              <TasksHistoryView entries={entries} project={activeProject} />
            </TabsContent>
