...
export function RichTextEditor({ value, onChange, placeholder, className, suggestions = [], disabled = false }: RichTextEditorProps) {
  ...
  const displayValue = presentationMode ? stripAllHours(value) : value;
  ...
  return (
    <div className={cn("space-y-0 relative group", disabled && "pointer-events-none", className)}>
      <textarea
        ref={textareaRef}
        className={cn(
          "flex min-h-[100px] w-full rounded-lg border border-border/60 bg-background/60 px-3.5 py-2.5 text-sm ring-offset-background placeholder:text-muted-foreground/50 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-primary/40 focus-visible:border-primary/40 focus-visible:bg-background disabled:cursor-not-allowed disabled:text-foreground disabled:opacity-100 font-mono leading-relaxed resize-none transition-all duration-200",
        )}
        value={displayValue}
      ...
      {!focused && hasContent && hasFormatting && (
        <div className="px-3.5 py-2 text-xs border border-t-0 border-border/40 rounded-b-lg bg-muted/10">
