...
import { stripActualHours, stripAllHours } from "@/lib/text-sanitizer";
...
function buildSections(yesterday: Entry | null, today: Entry | null, stripFn: (s: string) => string = stripActualHours) {
  return [
    {
      title: "Yesterday",
      content: yesterday?.done ? stripFn(yesterday.done) : "No entry",
      icon: CheckCircle2,
      color: "text-success",
    ...
    },
    {
      ...
      content: (today?.doing || today?.done)
        ? stripFn(today?.doing || "See today's entry")
        : "No entry yet",
      icon: ListTodo,
      color: "text-primary",
...
export function StandupView({ project, yesterday, today, allProjectsStandup }: StandupViewProps) {
  ...
  const stripFn = presentationMode ? stripAllHours : stripActualHours;
  const sections = buildSections(yesterday, today, stripFn);
