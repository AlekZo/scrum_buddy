...
import { isAIConfigured, mergeTasks, polishText, polishBlockers, AIError, getAISettings } from "@/lib/ai-service";
...
interface EntryFormProps {
  ...
  latestPreviousEntry?: Entry | null;
  planData: PlanData;
  onSave: (project: string, entry: Entry) => void;
  onUpdateDuplicateVersions?: (sourceEntry: Entry, version: string) => number;
}

export function EntryForm({ entry, date, project, previousTasks, yesterday, latestPreviousEntry, planData, onSave, onUpdateDuplicateVersions }: EntryFormProps) {
  const { t } = useI18n();
  ...
  const handlePrefillFromPrevious = () => {
    const source = latestPreviousEntry || yesterday;
    if (!source || isReported) return;
    setForm((prev) => ({
      ...prev,
      done: source.doing ? source.doing : prev.done,
    }));
    triggerAutoSave();
    toast.success("Prefilled 'What I did' from previous day's plan");
  };
    ...
    <div className="space-y-4">
      ...
      <Card className="border-border/50">
        <CardHeader className="pb-4">
          ...
          {(latestPreviousEntry?.doing || yesterday?.doing) && (
            <Button
              size="sm"
              variant="outline"
            ...
            >
              ...
              Prefill "What I did" from {latestPreviousEntry ? `${latestPreviousEntry.date}` : "yesterday"}'s plan
            </Button>
          )}
              ...
              {polishing.has(key) ? (
                ...
                <div className={isReported && key !== "blockers" ? "relative" : ""}>
                  {isReported && key !== "blockers" && (
                    <div className="absolute inset-0 z-10 rounded-lg cursor-not-allowed flex items-end justify-end p-2">
