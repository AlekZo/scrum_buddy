...
export function MatrixPlanningView({
  ...
  const [activePanel, setActivePanel] = useState<"backlog" | "buffer" | null>(null);
  ...
  const handleAutoFill = useCallback(
    (day: string) => {
      ...
      if (bufferedTasks.length === 0) {
        toast.info("Buffer is empty — add tasks first");
        return;
      }
      ...
      toast.success(`Auto-filled ${filled.toFixed(1)}h ✨`);
    },
  ...
  const togglePanel = (panel: "backlog" | "buffer") => {
    ...
    <TooltipProvider>
      <DragDropContext onDragEnd={handleDragEnd}>
        <div className="space-y-3">
            ...
            <div className="ml-auto flex items-center gap-1.5">
              ...
              >
                ...
                {backlogTasks.length > 0 && (
                  <Badge variant={activePanel === "backlog" ? "outline" : "secondary"} className="text-[9px] px-1 py-0 ml-0.5">
                    {backlogTasks.length}
                  </Badge>
                )}
              </Button>
              <Button
                variant={activePanel === "buffer" ? "default" : "outline"}
                size="sm"
                className="h-7 text-xs gap-1.5"
                onClick={() => togglePanel("buffer")}
              >
                <Vault className="w-3.5 h-3.5" />
                Buffer
                {bufferedTasks.length > 0 && (
                  <Badge variant={activePanel === "buffer" ? "outline" : "secondary"} className="text-[9px] px-1 py-0 ml-0.5 bg-success/10 text-success">
          ...
          {/* Main layout: optional inline side panel + matrix */}
          <div className="flex gap-3">
            {/* ── INLINE SIDE PANEL (replaces blocking Sheet overlay) ── */}
            {activePanel && (
              <div className="w-64 shrink-0 animate-in slide-in-from-left-4 fade-in duration-200">
                <Card className="border-border/50 sticky top-32">
                  <CardHeader className="p-3 pb-2">
                    ...
                    {/* Explanation */}
                    <div className="flex items-start gap-1.5 p-2 rounded-md bg-muted/50 mt-1">
                      <Info className="w-3 h-3 text-muted-foreground mt-0.5 shrink-0" />
                      <p className="text-[10px] text-muted-foreground leading-relaxed">
                        {activePanel === "backlog"
                          ? "Unscheduled tasks. Drag them onto day columns to schedule, or move to Buffer when done early."
                          : "Pre-completed work. Use ✨ Auto-fill on any day to report these hours, or drag onto days."
                        }
                      </p>
                    </div>
                  </CardHeader>
                  ...
                  <CardContent className="p-2 pt-0">
                    ...
                    <Droppable droppableId={activePanel === "backlog" ? "backlog" : "buffer"}>
                      {(provided, snapshot) => (
                        <div
                          ref={provided.innerRef}
                          {...provided.droppableProps}
                          className={cn(
                            "space-y-1 min-h-[80px] rounded-md p-1 transition-colors",
                            snapshot.isDraggingOver && activePanel === "backlog" && "bg-primary/5 ring-1 ring-primary/20",
                            snapshot.isDraggingOver && activePanel === "buffer" && "bg-success/5 ring-1 ring-success/20"
                          )}
                        >
                          {(activePanel === "backlog" ? backlogTasks : bufferedTasks).map((task, i) => (
                            <DrawerTaskCard
                              key={task.id}
                              task={task}
                              index={i}
                              variant={activePanel === "backlog" ? "solid" : "ghost"}
                              onRemove={onRemoveTask}
                              onBank={activePanel === "backlog" ? handleBankTask : undefined}
                              onUnbank={activePanel === "buffer" ? handleUnbankTask : undefined}
                            />
                          ))}
                          {provided.placeholder}
                          {(activePanel === "backlog" ? backlogTasks : bufferedTasks).length === 0 && (
                            <p className="text-[10px] text-muted-foreground/50 text-center py-6">
                              {activePanel === "backlog"
                                ? "No backlog tasks — add one above"
                                : "No buffered tasks — bank completed work here"
                              }
                            </p>
                          )}
                        </div>
                    ...
                    {activePanel === "backlog" && backlogTasks.length > 0 && (
                      <div className="mt-2 flex items-center gap-1 text-[9px] text-muted-foreground/60">
                        <ArrowRight className="w-3 h-3" />
                        Drag tasks to day columns to schedule
                      </div>
                    )}
                    {activePanel === "buffer" && bufferedTasks.length > 0 && (
                      <div className="mt-2 flex items-center gap-1 text-[9px] text-muted-foreground/60">
                        <Wand2 className="w-3 h-3" />
                        Click ✨ on any day to auto-fill
                      </div>
                    )}
                  </CardContent>
                </Card>
      ...
      {(provided, snapshot) => (
        <Tooltip>
                ...
                <div className="flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                  {!isGhost && (
                    <Tooltip>
                      ...
                      <TooltipContent side="top" className="text-[10px]">Move to Buffer</TooltipContent>
                    </Tooltip>
                  )}
                  <button
          ...
          <TooltipContent side="right" className="text-[10px] max-w-[200px]">
            ...
            {isGhost && <p className="text-success">↑ From buffer</p>}
          </TooltipContent>
        </Tooltip>
      )}
    </Draggable>
...
/* ── Panel task card (backlog / buffer) ── */
