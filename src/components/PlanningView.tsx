...
/** Parse a decimal string supporting both dot and comma as separators */
...
export function PlanningView({ project, planData, onSaveTask, onRemoveTask, onUpdateTask }: PlanningViewProps) {
  ...
  const handleBulkUpdateHours = () => {
    ...
    if (bulkTeamHours !== "") updates.teamHours = parseDecimal(bulkTeamHours);
    if (bulkActualHours !== "") updates.actualHours = parseDecimal(bulkActualHours);
...
function TaskItem({ task, isMultiDay, onUpdate, onRemove, isSelected, onToggle }: TaskItemProps) {
  ...
  const [teamHoursValue, setTeamHoursValue] = useState(String(task.teamHours || ""));
  const [actualHoursValue, setActualHoursValue] = useState(String(task.actualHours || ""));
  const [detailOpen, setDetailOpen] = useState(false);
  const [detailEndDate, setDetailEndDate] = useState(task.endDate);
  const nameInputRef = useRef<HTMLInputElement>(null);
  const teamHoursInputRef = useRef<HTMLInputElement>(null);
  ...
  useEffect(() => {
    if (!editingHours) {
      setTeamHoursValue(String(task.teamHours || ""));
      setActualHoursValue(String(task.actualHours || ""));
    }
  }, [task.teamHours, task.actualHours, editingHours]);
  ...
  const commitHours = () => {
    const teamVal = parseDecimal(teamHoursValue);
    const actualVal = parseDecimal(actualHoursValue);
    const updates: Partial<PlanTask> = {};
    if (teamVal !== task.teamHours) updates.teamHours = teamVal;
    if (actualVal !== task.actualHours) updates.actualHours = actualVal;
  ...
  useEffect(() => {
    if (editingHours) {
      teamHoursInputRef.current?.focus();
      teamHoursInputRef.current?.select();
    }
  }, [editingHours]);
  ...
  }, [detailOpen, task.endDate]);

  const displayHours = task.teamHours > 0 || task.actualHours > 0;
  const hoursLabel = task.teamHours > 0 && task.actualHours > 0 && task.teamHours !== task.actualHours
    ? `T:${task.teamHours} A:${task.actualHours}h`
    : task.teamHours > 0
    ? `${task.teamHours}h`
    : task.actualHours > 0
    ? `~${task.actualHours}h`
  ...
  return (
    ...
    >
      ...
      {/* Hours — inline editable (Team + Actual) */}
      {editingHours ? (
        <div className="flex items-center gap-0.5 shrink-0">
          <input
            ref={teamHoursInputRef}
            inputMode="decimal"
            placeholder="Team"
            value={teamHoursValue}
            onChange={(e) => setTeamHoursValue(e.target.value)}
            onBlur={commitHours}
            onKeyDown={(e) => {
              if (e.key === "Enter") commitHours();
              if (e.key === "Escape") { setTeamHoursValue(String(task.teamHours || "")); setActualHoursValue(String(task.actualHours || "")); setEditingHours(false); }
            }}
            className="w-11 bg-secondary/50 border border-border/50 rounded text-[10px] font-mono text-center outline-none focus:ring-1 focus:ring-primary/50 py-0.5"
          />
...
function WeekGridView({
  ...
  const handleAddMultiTask = () => {
    ...
    const explicitHours = parseDecimal(multiTaskHours);
    const teamHours = explicitHours > 0 ? explicitHours : parsed.teamHours;
